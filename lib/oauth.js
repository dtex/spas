...
manager.addJob('finishAuth', {
	work: function( tout, bid, key, cb, authParams ) {
		...
		log.info('Run job oauth:finishAuth');
		cb(tout, authParams);
		this.finished = true;
	}
});
...
manager.addJob('getTemporaryCredentials', {
	work: function( api, bid, key, cb ) {
		...
		log.info ('getTemporaryCredentials called for ' + bid + ', ' + key);
        ...
        oa.getOAuthRequestToken(function(error, oauth_token, oauth_token_secret, results){
            if(error) {
	            log.error(error);
	            manager.enqueue('finishAuth', error, bid, key, cb);
	            self.finished = true;
	        } else { 
		        api.credentials = {
            	...
            	var tout = {
	  			...
	  			log.debug('Authorize redirect response:' + JSON.stringify(tout));
...
manager.addJob('getAccessToken', {
	work: function(api, bid, key, oauth_token, oauth_verifier, cb) {
		...
		log.info('Running getAccessToken for ' + bid + ', ' + key);
		var oaData = GLOBAL.config.authentication[api.auth.provider],
			self = this;
		...
		var oa = new oauth(oaData.requestTemporaryCredentials,
			  ...
			  log.debug('oauth_token:' + oauth_token + ' api.credentials.oauth_token_secret:' + api.credentials.oauth_token_secret);
...
exports.authorize = function( api, bid, key, cb ) {
	...
	log.info('oauth authorize called for ' + bid +', ' + key);
	
	if (!_.has(GLOBAL.config, 'authentication') || !_.has(GLOBAL.config.authentication, api.auth.provider)) {
		log.error('Authentication provider ' + api.auth.provider + ' not defined');
		return false;
	}
	...
	client.get(bid+key+'oauth', function (err, doc) {
		...
		if (err || doc === null) { 
			...
			if (_.has(api.credentials, 'oauth_access_token')) {
				manager.enqueue('finishAuth', true, bid, key, cb, {
					"oauth_consumer_key": api.credentials.oauth_consumer_key, 
					//"oauth_nonce": bid+","+key,
					//"oauth_signature": xxx,
					"oauth_signature_method": oaData.encryption,
...
exports.saveOauthToken = function( api, oauth_token, oauth_verifier, bid, key, cb) {
	...
	log.info('Running saveOauthToken for ' + bid + ', ' + key);
