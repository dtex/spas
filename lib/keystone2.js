...
manager.addJob('getTokens', {
	work: function( api, bid, key, cb ) {
		log.verbose('--- getTokens ---');
		var oaData = config.authentication[api.auth.provider],
			self = this;
		log.verbose('code='+api.credentials.code+
		...
		request(
			...
			function (err, myres, body) {
				log.verbose('getTokens response', body);
				...
				log.verbose('SET', bid+key+'oauth2', JSON.stringify(api.credentials));
...
manager.addJob('refreshTokens', {
	work: function( api, bid, key, cb ) {
		log.verbose('--- refreshTokens ---');
		log.verbose('api', api);
		
		var oaData = config.authentication[api.auth.provider],
			self = this;
		log.verbose(oaData.oauth2token);
		log.verbose('client_id='+oaData.client_id+
					'&client_secret='+oaData.client_secret+
		...
		request(
			...
			function (err, myres, body) {
				log.verbose('refreshTokens response', body);
				var jsRes = JSON.parse(body);
...
exports.saveCode = function( res, state, code) {
	log.verbose('--- saveCode ---');
	var doc = {"code": code };
	log.verbose('SET', state[0]+state[1]+'oauth2', JSON.stringify(doc));
...
exports.authorize = function( api, bid, key, cb ) {
	...
	client.get(bid+key+'oauth2', function (err, doc) {
		if (err || doc === null) { 
			...
			log.verbose(bid+key+'oauth2', doc);
			...
			if (_.has(api.credentials, 'access_token')) {
				log.verbose('has access_token');
				if (api.credentials.expires.isBefore(new Date())) {
					log.verbose('It has expired. Calling refreshTokens');
