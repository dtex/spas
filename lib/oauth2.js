...
manager.addJob('finishAuth', {
	work: function( tout, bid, key, cb, authParams ) {
...
manager.addJob('getCode', {
	work: function( api, bid, key, cb ) {
		...
		log.info('Run job oauth2:getCode');
		...
		log.debug(JSON.stringify(tout));
		
		manager.enqueue('finishAuth', tout, bid, key, cb);
		self.finished = true;		        				   
	}
});
...
manager.addJob('getAccessToken', {
	...
	work: function(api, bid, key, cb, grant_type) {
		...
		log.info('Run job oauth2:getAccessToken(' + grant_type + ')');
		var oaData = GLOBAL.config.authentication[api.auth.provider],
			self = this;
		var oa = new oauth2(oaData.client_id,
			oaData.client_secret,
        ...
        log.debug('api = ' + JSON.stringify(api));    
        log.info('oaData = ' + JSON.stringify(oaData));
        ...
        log.info('thisCode = ' + thisCode);
        log.info('params  = ' + JSON.stringify(params));
        
        oa.getOAuthAccessToken(thisCode, params, function(error, access_token, refresh_token, results) {
	    	log.info('Run callback for oauth2:getOAuthAccessToken');
	    	if(error) {
		        log.error('Error oauth2:getOAuthAccessToken('+bid+key+'): '+JSON.stringify(error));
		        var tout = {
	  				expires: new Date(),
	  				err: error,
	  				cname: key
	        	...
	        	log.debug('typeof refresh_token = '+typeof refresh_token);
	        	if (typeof refresh_token !== 'undefined') {
	        		log.debug('first refresh token = '+refresh_token);
	        		api.credentials.refresh_token = refresh_token;
	        	}
	        	
	        	api.credentials.expires = new Date().add({seconds: (results.expires_in - 300)});
	        	
	        	log.debug(JSON.stringify(api.credentials));

	        	client.set(bid+key+'oauth2', JSON.stringify(api.credentials));
	  			log.debug(bid+key+'oauth2 saved');
	  			manager.enqueue('finishAuth', true, bid, key, cb, { "access_token": api.credentials.access_token });
	  			self.finished = true;
	  		}
	   });
	}
...
exports.authorize = function( api, bid, key, cb ) {
	...
	log.info('function oauth2:authorize');
...
exports.saveCode = function( res, state, code, cb) {
	var bid = state[0], key = state[1];
	var log = logger.child({bid: bid, key: key});

	log.info('function oauth2:saveCode');
	
	var doc = {"code": code };
	client.set(bid+key+'oauth2', JSON.stringify(doc));
	var tout = {
		expires: new Date(),
