...
exports.fulfill = function ( myRes, ip, bid, callback, gzip, override ) {
	...
	if (!callback && bundle && bundle.callback) {
		...
		client.get('bid'+bid, function ( err, doc ) {	
			...
			if ( err || doc === null ) {
				...
				if (Number(jDoc.secleft) < 0 ) {
					...
					var responseHeaders = {'Content-Type': responseType, 'vary': 'Accept-Encoding', 'max-age': jDoc.secleft, 'cache-control': 'public, max-age='+jDoc.secleft+', no-transform', "Expires": jDoc.expires };
					...
					if (gzip) {
						...
						zlib.gzip(tbuf.toString(), function(err, zbuf) {
						  if (!err) {
						    ...
						    myRes.writeHead(200, {'Content-Type': responseType, 'vary': 'Accept-Encoding', 'max-age': jDoc.secleft, 'cache-control': 'public, max-age='+jDoc.secleft+', no-transform', "Expires": jDoc.expires, 'content-encoding': 'gzip' });
						    myRes.end(zbuf);
						  } 
						});
					} else {
		...
		manager.addJob('sendResponse', {
			work: function(doc) {
				...
				if (_.has(doc, 'redirect')) {
					...
					var responseType = callback ? 'application/javascript' : 'application/json';
					var responseHeaders = {'Content-Type': responseType, 'vary': 'Accept-Encoding', 'max-age': doc.secleft, 'cache-control': 'public, max-age='+doc.secleft+', no-transform', "Expires": doc.expires };
					...
					if (gzip) {
						...
						winston.event('Send response for ' + bid +', ' + tbuf.length + ', ' + ip);
						myRes.writeHead(200, responseHeaders);
						myRes.end(tbuf);
						this.finished = true;
					}
