		...
		_.each(bundle, function (api, key) {
			if (api.schedule) {
				var job = new cronJob(api.schedule, function(){
		    		...
		    		exports.refresh(api, key, bid, bundle);  
		    	}, null, true);
		    	cronjobs.push(job);
		    }
		});
...
//
exports.fulfill = function ( myRes, ip, bid, bundle, callback, gzip, override ) {
	...
	if( _.isUndefined( override )) {
		...
		client.get('bid'+bid, function ( err, doc ) {	
			...
			if ( err || doc === null ) {
				...
				exports.fulfill( myRes, ip, bid, bundle, callback, gzip, true );
		...
		// ### Override was passed so we are forcing a refresh
		...
		manager.addJob('fulfillPart', {
			work: function(api, bid, key, override, cachedPart) {
				...
				winston.info('manager:fulfillPart: ' + bid + '.' + key);
				...
				if ( _.isUndefined( override ) ) {
					...
					if (_.has( api, 'auth')) { 
						...
						api.auth.type.authorize (api, bid, key, function( result, authParams ) { 
							if (result === true) {
