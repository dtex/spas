...
// Configure loggly if defined
if (GLOBAL.config.loggly) {
  var logglyClient = loggly.createClient({
    token: "fbeec99c-724e-418c-937c-5f180676e06b",
    subdomain: "donovan",
    tags: ["NodeJS"],
    json:true
  });
...
var sendResponse = function(jDoc, myRes, ip, bid, callback, gzip) {
	...
	if (Number(jDoc.secleft) < 0 && !GLOBAL.bundles[bid].locked) {
		...
		if (gzip) {
			...
			zlib.gzip(doc, function(err, zbuf) {
			  if (!err) {
			    ...
			    if (logglyClient) {
            logglyClient.log('Send gzipped response for ' + bid +', ' + zbuf.toString().length + ', ' + ip);
          }
      ...
      if (logglyClient) {
        logglyClient.log('Send response for ' + bid +', ' + doc.length + ', ' + ip);
      }
      myRes.end(tbuf);
		}
...
exports.refresh = function(api, key, bid, bundle) {
  ...
  winston.info('exports.refresh: ' + api);
	...
	api.resource( api.params, api.credentials, function( err, res ) {
        ...
        if (logglyClient) {
          logglyClient.log('Get data for ' + bid +' from ' + key + ', ' + res.size);
        }
